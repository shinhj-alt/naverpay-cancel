...
async function waitForLogin(page) {
  ...
  while (Date.now() - start < CFG.loginTimeoutMs) {
    ...
    if (u.startsWith('https://admin.pay.naver.com/') && !/login|nid/i.test(u)) {
      // SSO 리다이렉트 중간에 admin URL을 잠깐 거칠 수 있으므로
      // 3초 후에도 같은 URL이면 진짜 로그인 완료로 판단
      await page.waitForTimeout(3000);
      const u2 = page.url();
      if (u2.startsWith('https://admin.pay.naver.com/') && !/login|nid/i.test(u2)) {
        // 한 번 더 확인 (2FA 리다이렉트 대비)
        await page.waitForLoadState('domcontentloaded', { timeout: 5000 }).catch(() => {});
        const u3 = page.url();
        if (u3.startsWith('https://admin.pay.naver.com/') && !/login|nid/i.test(u3)) {
          jobLog('✅ 로그인 감지!');
          return;
        }
      }
...
async function runAutomation(input) {
  ...
  try {
    ...
    jobLog(`══════════════════════════════════════`);
    jobLog('브라우저는 직접 닫으시면 됩니다.');

  } finally {
    // 브라우저를 자동으로 닫지 않음 — 사용자가 직접 닫거나 10분 후 자동 정리
    if (ctx) {
      const autoClose = setTimeout(() => ctx.close().catch(() => {}), 10 * 60 * 1000);
      ctx.once('close', () => clearTimeout(autoClose));
    }
  }
}
